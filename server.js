...
// ── API OFICIAL v2 (via helper ca-oauth) — feature flag ──
// Default OFF: enquanto false, tudo continua na API interna X-Auth (comportamento atual).
// Quando ON, as criações vão pra API oficial v2 ATRAVÉS do helper ca-oauth (que já tem o
// token OAuth e renova sozinho) — sem OAuth/credencial próprios aqui.
const USE_OFFICIAL_API = String(process.env.USE_OFFICIAL_API || '').toLowerCase() === 'true';
const HELPER_URL = (process.env.HELPER_URL || 'https://ca-oauth.187.77.238.125.sslip.io').replace(/\/$/, '');
...
// Chamada à API oficial v2 ATRAVÉS do helper ca-oauth (que detém o token OAuth e o renova
// sozinho). O helper expõe GET|POST /api?path=/v1/... (Basic auth = ADMIN_TOKEN) e repassa
// pra api-v2.contaazul.com, devolvendo o status REAL da Conta Azul. Sem credencial própria aqui.
async function caOfficialRequest(method, apiPath, body) {
  if (!HELPER_ADMIN_TOKEN) throw new Error('HELPER_ADMIN_TOKEN ausente (configurar no Coolify)');
  const [basePath, query] = apiPath.split('?');
  let helperUrl = `${HELPER_URL}/api?path=${encodeURIComponent(basePath)}`;
  if (query) helperUrl += `&${query}`;
  const auth = Buffer.from(`admin:${HELPER_ADMIN_TOKEN}`).toString('base64');
  return httpsJson(method, helperUrl, {
...
const server = http.createServer(async (req, res) => {
  ...
  // (OAuth do ca-push removido — a API oficial v2 é acessada via helper ca-oauth, ver caOfficialRequest)
